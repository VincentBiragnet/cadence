...
- [x] **VC-4** Given {"durationSeconds": 5} with no frequencies at all, the clock runs to completion with zero oscillator starts (G-1, G-5) `npx playwright test tests/config.spec.js -g 'no frequency'` → passed 2026-08-21 (ran: exit 0 — Running 1 test using 1 worker ✓ 1 tests/config.spec.js:3:5 › no frequency: completes with ), passed 2026-08-22 (ran: exit 0 — Running 1 test using 1 worker ✓ 1 tests/config.spec.js:3:5 › no frequency: completes with )
