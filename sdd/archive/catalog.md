...
## program
**Archived:** 2026-08-22 · **Status:** Done · **struck, not passed:** VC-1 · **Commit:** 1dcdb22476837fe31d0afffa4da257e457dbec6a `specs/program.spec.md`

Built <cadence-program>: a title and a list of entries, each pairing a planned ISO-8601 datetime with a nested sequence-clock config ({ title, blocks }). A native <select> dropdown lists every entry unfiltered in JSON order, pre-selected to the soonest planned entry that hasn't been run yet (a done entry is never re-suggested). Picking one and clicking Start reveals a real, visible <cadence-sequence> in place of the list, with a Back affordance to abandon the whole session with nothing recorded; letting it finish records actualDatetime = now() on that entry, fires cadence:entryComplete, and returns to the (recomputed) list showing that entry as done — closing the planned-vs-actual loop the very first spec's premise described. No backend, no localStorage: an Export button downloads the current state (including actualDatetime fields) as JSON, and a file-input Load reads one back in, fully replacing prior state. Accessibility: native semantic controls throughout, aria-live announcing start/completion only. Verified: suggested-entry selection (both the soonest-overall case and the never-resuggest-done case), unfiltered list order, the start/back/complete hand-off, aria-live announcements, and the export/load round-trip actually replacing (not merging into) prior state. Explicitly deferred: missed-entry handling (a passed planned datetime with no actual run) — non-goal for this pass.
...
## exclusive-views-restored-focus-reachable-controls
