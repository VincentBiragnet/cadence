...
test('confirming a drop marks it, moves nothing, and stops it being suggested', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      label: prog.querySelectorAll('.cdp-row')[0].getAttribute('aria-label'),
      suggested: prog.querySelector('.cdp-row[aria-selected=\"true\"]').getAttribute('aria-label'),
...
// The irreversible control is not the neighbour of the daily one: it is not
// on the visible surface at all until the menu is opened (KD-14).
test('Drop lives behind the More menu, not beside Start', async ({ page }) => {
  await page.goto('/tests/fixture.html');
  const shape = await page.evaluate((p) => {
    const prog = document.createElement('cadence-program');
    document.body.appendChild(prog);
    prog.configure(p);
    return {
      ...
      menuClosed: prog.querySelector('.cdp-pop').hidden,
      dropVisible: prog.querySelector('.cdp-drop').getBoundingClientRect().height > 0,
    };
  }, PROGRAM);

  expect(shape.actions).not.toContain('cdp-drop');
  expect(shape.dropInMenu).toBe(true);
  expect(shape.menuClosed).toBe(true);
  expect(shape.dropVisible).toBe(false);
});
